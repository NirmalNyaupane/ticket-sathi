...
class UserResolver {
    ...
    @UseMiddleware(authentication())
    async updateUser(@Arg("data") userData: UpdateUserValidation, @Ctx() context: Context): Promise<CommonResponse> {
        const updateData = {} as Omit<UpdateUserValidation, "oldPassword" | "newPassword"> & { password: string };
        // const updatedResponse = await userService.
        if (userData.oldPassword) {
            const user = await authService.findTokenById(context.user?.id as string);
            const isPasswordCorrect = await bcrypt.compare(userData.oldPassword, user?.password as string);
        ...
        const updatedResponse = await userService.updateUserById(context.user?.id as string, updateData);
