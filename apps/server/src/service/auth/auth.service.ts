...
class AuthService {
  ...
  async registerUser(data: RegisterValidator) {
    ...
    if (user.role === UserRole.ORGANIZER) {
      ...
      const documentsIds = data.documents.map((doc) => doc.id);
      const media = await mediaService.findMultipleMediabyIds(documentsIds);
      if (!media) {
      ...
      if (media?.includes(null)) {
        throw (
          (new CustomError(
            "One or more documents are not available (document id is wrong)"
          ),
          HTTPStatusCode.BAD_REQUEST)
        );
      }
      ...
      for (let singleMedia of media) {
        if (singleMedia?.mediaType !== MediaType.ORGANIZER_DOCUMENT) {
          throw new CustomError(
            `All documents should be a tyepe of ${MediaType.ORGANIZER_DOCUMENT}`,
