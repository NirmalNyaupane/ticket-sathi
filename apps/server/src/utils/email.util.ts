    ...
    switch (mailType) {
      case MailType.REGISTRATION: {
        const source = fs.readFileSync(
          `${__dirname}/../templates/newRegistration.html`,
          "utf-8"
        );
      ...
      case MailType.FORGOT_PASSWORD: {
        const source = fs.readFileSync(
          `${__dirname}/../templates/linkbasedverification.html`,
          "utf-8"
        );
        const template = Handlebars.compile(source);
        const data = {
      ...
      case MailType.CHANGE_STATUS: {
        const source = fs.readFileSync(
          path.join(`${__dirname}`, "utils", "templates", "changestatus.html"),
          "utf-8"
        );
        const template = Handlebars.compile(source);
        const data = {
          subject: mailOption.subject,
          name: mailOption.data.name,
          title: mailOption.data.title,
          message: mailOption.data.message,
        };
        const body = template(data);
        return body;
      }
